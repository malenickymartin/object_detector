...
class AddRenders(nn.Module):
    def __init__(self, train_dataset, aug_dataset, max_train_objs = 5, max_aug_objs = 5):
        super().__init__()
        ...
        assert set(self.train_names).isdisjoint(self.aug_names), "Train and augumentation datasets have object with same name (label)."
        ...
        for object in self.train_names:
            ...
            self.train_objects.append({"label": object,
                                    "renders":list(sorted(RENDERS_PATH(train_dataset, object).iterdir())),
                                    "masks":list(sorted(MASKS_PATH(train_dataset, object).iterdir()))})
            assert len(self.train_objects[-1]["renders"]) == len(self.train_objects[-1]["masks"]), f"Number of masks and renders dont match for object {object}."

        for object in self.aug_names:
            if RENDERS_PATH(aug_dataset, object).is_dir():
                self.aug_objects.append({"label": object,
                                        "renders":list(sorted(RENDERS_PATH(aug_dataset, object).iterdir())),
                                        "masks":list(sorted(MASKS_PATH(aug_dataset, object).iterdir()))})
                assert len(self.aug_objects[-1]["renders"]) == len(self.aug_objects[-1]["masks"]), f"Number of masks and renders dont match for object {object}."
    ...
    def forward(self, render, mask, curr_obj_name):
        ...
        rand_train_objs = np.random.randint(0, min(self.max_train_objs, len(data_objects)) + 1) 
        rand_aug_objs = np.random.randint(0, min(self.max_aug_objs, len(self.aug_objects)) + 1)
