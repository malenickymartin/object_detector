...
class AddRenders(nn.Module):
    def forward(self, render, target, object_name):
        mask = target["masks"]
        mask = np.array(mask)
        mask = mask > 0
        mask = np.transpose(mask, (1,2,0))
        only_object_mask = np.array(mask)
        ...
        for object in objects_dir:
            if os.path.isdir(RENDERS_PATH(object)) and object != object_name:
        ...
        for object in np.random.choice(objects, np.random.randint(0, len(objects)+1), replace=False):
            ...
            if np.random.choice([True, False]) and ovelay_rate < 0.75:
                ...
                render = np.where(mask, render, new_render)
            mask = np.logical_or(new_mask, mask)
        return render, mask, target
    
class AddBackground(nn.Module):
    def forward(self, render, mask, target):
        ...
        result = np.where(mask, render, background)
